...
class WinPollster(object):
    ...
    def get_mem(self):
        ...
        data_dict["Cached"] = {'volume': round(float(winproc_mem['cached'])  / (1024*1024), 2), 'unit':'MB'}
        data_dict["Buffers"] = {'volume': round(float(winproc_mem['buffers'])  / (1024*1024), 2), 'unit':'MB'}
